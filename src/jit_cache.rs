...
//! device buffers, so no specific topology is baked in, but the state array
//! is sized at compile time by `-D QUIP_MAX_NODES`. One cached PTX per
//! (source, GPU arch, driver/NVRTC version, node capacity) therefore serves
//! every topology and model that fits that capacity.
//!
